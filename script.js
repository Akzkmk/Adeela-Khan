document.addEventListener('DOMContentLoaded', () => {
  ...
  class Streak {
    ...
    draw() {
      ...
      const g = this.ctx.createLinearGradient(this.x, this.y, this.x - this.len, this.y);
      g.addColorStop(0, `hsla(${this.hue}, 100%, 70%, ${alpha})`);
  ...
  // --- Three.js Futuristic Agentic Core (Updated) ---
  ...
  if(container && THREE){
    ...
    // Particle system for "data packets" (New)
    ...
    // --- Holographic scan lines overlay (Unchanged) ---
    ...
    function animate() {
      ...
      // Orbit nodes & particle pulses
      nodes.forEach((n) => {
        n.angle += 0.0008;
        n.mesh.position.set(Math.cos(n.angle) * orbitRadius, Math.sin(n.angle * 0.5) * 3, Math.sin(n.angle) * orbitRadius);
        
        // Node pulse if a particle is near
        const nearestParticleDistance = particlePool.reduce((minDist, p) => 
          Math.min(minDist, p.mesh.position.distanceTo(n.mesh.position)), Infinity);
        
        n.mesh.material.emissiveIntensity = n.baseIntensity + (nearestParticleDistance < 2 ? 0.8 : 0);
      });
